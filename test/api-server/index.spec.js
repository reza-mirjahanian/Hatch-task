    ...
    suite('POST /api/posts', () => {
        test('should return error, If `tags` parameter is not present ', async () => {
            const {
                data: response,
                status
            } = await axios.get(`${SERVER_URL}/api/posts?&sortBy=likes&direction=desc`,{    validateStatus: function () {
                    return true;
                }});
        ...
        test('should return error, If `direction` is invalid value', async () => {
            //@todo fix stub
            const {
                data: response,
                status
            } = await axios.get(`${SERVER_URL}/api/posts?tags=history,tech&sortBy=likes&direction=up`,{    validateStatus: function () {
                    return true;
                }});
            expect(status).to.equal(400)
        ...
        test('should return error, If `sortBy` is invalid value', async () => {
            //@todo fix stub
            const {
                data: response,
                status
            } = await axios.get(`${SERVER_URL}/api/posts?tags=history,tech&sortBy=price&direction=asc`,{    validateStatus: function () {
                    return true;
                }});
            expect(status).to.equal(400)
            expect(response.error).to.equal("sortBy parameter is invalid")
